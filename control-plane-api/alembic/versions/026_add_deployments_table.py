...
def upgrade() -> None:
    ...
    # webhookeventtype enum was referenced here but never created by migration 003
    # (webhook_deliveries.event_type uses String(100), not an enum).
    # Skipped to avoid failure on databases where the enum does not exist.
    has_enum = conn.execute(
        sa.text("SELECT 1 FROM pg_type WHERE typname = 'webhookeventtype'")
    ).fetchone()
    if has_enum:
        op.execute("COMMIT")
