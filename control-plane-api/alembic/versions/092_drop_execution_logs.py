...
Feature removed: execution logs are redundant with Call Flow live traces (Prometheus).
"""
...
def downgrade() -> None:
    executionstatus = postgresql.ENUM(
        "success", "error", "timeout", name="executionstatus", create_type=False
    )
    errorcategory = postgresql.ENUM(
        "auth", "rate_limit", "backend", "timeout", "validation",
