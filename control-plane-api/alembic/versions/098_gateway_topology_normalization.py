...
Revision ID: 098_gateway_topology_norm
...
revision: str = "098_gateway_topology_norm"
        ...
        sa.text("""
            ...
            SET
              target_gateway_type = CASE
                WHEN gateway_type::text IN ('webmethods', 'kong', 'gravitee', 'apigee', 'aws_apigateway', 'azure_apim')
                  THEN gateway_type::text
              ...
              deployment_mode = CASE
                WHEN mode IN ('edge-mcp', 'edge', 'mcp') OR gateway_type::text = 'stoa_edge_mcp' THEN 'edge'
                ELSE 'connect'
              END,
              topology = CASE
                WHEN mode IN ('edge-mcp', 'edge', 'mcp') OR gateway_type::text = 'stoa_edge_mcp' THEN 'native-edge'
                ELSE 'remote-agent'
              END,
