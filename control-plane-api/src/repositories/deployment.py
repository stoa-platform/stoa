...
class DeploymentRepository:
    ...
    async def get_by_id_and_tenant(self, deployment_id: UUID, tenant_id: str) -> Deployment | None:
        result = await self.db.execute(
    ...
    async def list_by_tenant(
        ...
        count_query = select(func.count()).select_from(Deployment).where(Deployment.tenant_id == tenant_id)
        if api_id:
    ...
    async def get_latest_success(self, tenant_id: str, api_id: str, environment: str) -> Deployment | None:
        result = await self.db.execute(
            select(Deployment).where(
                Deployment.tenant_id == tenant_id, Deployment.api_id == api_id,
                Deployment.environment == environment, Deployment.status == DeploymentStatus.SUCCESS.value,
            ).order_by(Deployment.completed_at.desc()).limit(1)
        )
        return result.scalar_one_or_none()

    async def get_environment_summary(self, tenant_id: str, environment: str) -> list[Deployment]:
        subquery = (
            select(Deployment.api_id, func.max(Deployment.created_at).label("latest"))
            .where(Deployment.tenant_id == tenant_id, Deployment.environment == environment)
            .group_by(Deployment.api_id).subquery()
        )
        result = await self.db.execute(
            select(Deployment).join(
                subquery, (Deployment.api_id == subquery.c.api_id) & (Deployment.created_at == subquery.c.latest),
            )
        )
