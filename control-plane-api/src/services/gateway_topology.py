...
def normalize_mode(mode: str | None) -> str:
    ...
    mode_lower = (mode or "").strip().lower().replace("_", "-")
...
def _normalize_deployment_mode(value: str | None) -> str | None:
    normalized = _normalized(value)
    return normalized if normalized in DEPLOYMENT_MODES else None


def _normalize_topology(value: str | None) -> str | None:
    normalized = _normalized(value)
...
def derive_target_gateway_type(
    ...
    *,
    ...
    """
    explicit = (explicit_target or "").strip().lower().replace("-", "_")
    ...
    gw_type = wire_value(gateway_type).strip().lower()
...
def normalize_gateway_topology(
    *,
    ...
    proof = (health_details or {}).get("topology_proof")
