...
class TestClassifySingleError:
    ...
    def test_classify_auth_error(self) -> None:
        log = _make_log(status_code=401, error_category=ErrorCategory.AUTH, error_message="JWT expired")
    ...
    def test_classify_rate_limit(self) -> None:
        log = _make_log(status_code=429, error_category=ErrorCategory.RATE_LIMIT, error_message="quota exceeded")
        cause = self.svc._classify_single(log)
        assert cause is not None
    ...
    def test_classify_circuit_breaker_open(self) -> None:
        log = _make_log(status_code=503, error_category=ErrorCategory.CIRCUIT_BREAKER, error_message="CB open")
        cause = self.svc._classify_single(log)
        assert cause is not None
    ...
    def test_classify_backend_error(self) -> None:
        log = _make_log(status_code=502, error_category=ErrorCategory.BACKEND, error_message="upstream unreachable")
        cause = self.svc._classify_single(log)
        assert cause is not None
    ...
    def test_classify_certificate_error(self) -> None:
        log = _make_log(status_code=502, error_category=ErrorCategory.CERTIFICATE, error_message="TLS handshake failed")
        cause = self.svc._classify_single(log)
        assert cause is not None
    ...
    def test_classify_policy_denial(self) -> None:
        log = _make_log(status_code=403, error_category=ErrorCategory.POLICY, error_message="OPA policy denied")
        cause = self.svc._classify_single(log)
        assert cause is not None
...
class TestConnectivity:
    ...
    async def test_connectivity_gateway_not_found(self) -> None:
        ...
        with patch.object(svc, "_get_gateway", return_value=None):
            result = await svc.check_connectivity(tenant_id="t1", gateway_id="nonexistent")
...
class TestFalsePositiveRate:
    ...
    TEST_CASES = [
        ...
        (429, ErrorCategory.RATE_LIMIT, "Too many requests", DiagnosticCategory.RATE_LIMIT),
        ...
        (504, ErrorCategory.TIMEOUT, "Gateway timeout", DiagnosticCategory.CONNECTIVITY),
        (504, None, "Read timed out", DiagnosticCategory.CONNECTIVITY),
        (503, ErrorCategory.CIRCUIT_BREAKER, "Circuit open", DiagnosticCategory.CIRCUIT_BREAKER),
        (500, None, "circuit breaker tripped", DiagnosticCategory.CIRCUIT_BREAKER),
        (502, ErrorCategory.CERTIFICATE, "SSL certificate verify failed", DiagnosticCategory.CERTIFICATE),
        ...
        (502, ErrorCategory.NETWORK, "DNS resolution failed", DiagnosticCategory.CONNECTIVITY),
        ...
        (429, ErrorCategory.RATE_LIMIT, "consumer quota exhausted", DiagnosticCategory.RATE_LIMIT),
    ]
    ...
    def test_false_positive_rate_below_10pct(self) -> None:
        ...
        assert false_positive_rate < 0.10, (
            f"False positive rate {false_positive_rate:.1%} exceeds 10% threshold " f"({correct}/{total} correct)"
        )
