...
class TestVaultCredentials:
    ...
    def test_create_server_vault_failure_graceful(self, app_with_cpi_admin, mock_db_session):
        """Vault failure during create is handled gracefully — server still created."""
        mock_srv = _mock_server()
    ...
    def test_update_server_vault_failure_graceful(self, app_with_cpi_admin, mock_db_session):
        """Vault failure during update is handled gracefully — server still updated."""
        server_id = uuid4()
        mock_srv = _mock_server(id=server_id)
    ...
    def test_test_connection_vault_failure_graceful(self, app_with_cpi_admin, mock_db_session):
        """Vault failure during test-connection falls back to testing without credentials."""
        server_id = uuid4()
        auth_mock = MagicMock()
        auth_mock.value = "bearer_token"
        transport_mock = MagicMock()
        ...
        mock_result.error = None
        mock_result.server_info = {}
        mock_result.tools_discovered = 0
    ...
    def test_sync_tools_vault_failure_graceful(self, app_with_cpi_admin, mock_db_session):
        """Vault failure during sync-tools falls back to syncing without credentials."""
        server_id = uuid4()
        auth_mock = MagicMock()
        auth_mock.value = "bearer_token"
        transport_mock = MagicMock()
        ...
        with (
            ...
            patch(VAULT_PATH) as MockVault,
            patch(MCP_CLIENT_PATH) as MockMCPClient,
            patch(TOOL_REPO_PATH),
        ):
            MockRepo.return_value.get_by_id = AsyncMock(return_value=mock_srv)
            MockRepo.return_value.sync_tools = AsyncMock(return_value=(0, 0))
            MockVault.return_value.retrieve_credential = AsyncMock(side_effect=Exception("vault down"))
            MockMCPClient.return_value.list_tools = AsyncMock(return_value=[])
