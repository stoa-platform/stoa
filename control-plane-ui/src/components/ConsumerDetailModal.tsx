...
export function ConsumerDetailModal({ consumer, tenantId, onClose }: Props) {
  ...
  const handleBind = useCallback(async () => {
    ...
    setBinding(true);
    try {
      await apiService.bindCertificate(tenantId, consumer.id, pemValue.trim());
      toast.success('Certificate bound', `Certificate successfully bound to ${consumer.name}`);
      invalidate();
      onClose();
    } catch (err: unknown) {
      toast.error('Bind failed', err instanceof Error ? err.message : 'Failed to bind certificate');
    } finally {
      setBinding(false);
    }
        ...
        >
          ...
          <div className="p-6 space-y-6">
            ...
            <section className="space-y-2">
              ...
              {hasCert && canBind && canEdit && !showBindForm && (
                <div className="mt-2">
                  <button
                    onClick={() => setShowBindForm(true)}
                    className="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400"
                  >
                    {certStatus === 'revoked' ? 'Rebind Certificate' : 'Replace Certificate'}
                  </button>
                </div>
              )}
            </section>
            ...
            {showBindForm && (
              <section className="space-y-3 border-t border-neutral-200 dark:border-neutral-700 pt-4">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={handleBind}
                    disabled={binding || !pemValue.trim()}
