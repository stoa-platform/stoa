/**
 * Credential Mappings Page (CAB-1648)
 *
 * Manages consumer→API backend credential mappings.
 * Ported from Portal's CredentialMappingsPage.tsx to Console.
 * cpi-admin / tenant-admin: full CRUD. devops / viewer: read-only list.
 */
...
export function CredentialMappings() {
  ...
  const isWriteUser = useMemo(() => {
    return hasRole('cpi-admin') || hasRole('tenant-admin');
  }, [hasRole]);

  // Load tenants for admin
  useEffect(() => {
    if (!isAdmin) return;
    apiService.getTenants().then((t) => setTenants(t));
  }, [isAdmin]);
  ...
  useEffect(() => {
    ...
    async function loadMappings() {
      ...
      setError(null);
      try {
        const result = await apiService.getCredentialMappings(tenantId!);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        if (modal.mode === 'create') {
          ...
          await apiService.updateCredentialMapping(tenantId, modal.editId, payload);
          toast.success('Credential mapping updated');
        }
        closeModal();
        handleRefresh();
      } catch (err) {
    ...
    [tenantId, modal, formData, closeModal, toast]
  );
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await apiService.deleteCredentialMapping(tenantId, deleteTarget.id);
      setMappings((prev) => prev.filter((m) => m.id !== deleteTarget.id));
      toast.success('Credential mapping deleted');
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Empty state */}
      {!isLoading && !error && tenantId && filteredMappings.length === 0 && (
        <div className="text-center py-12">
          <Shield className="h-12 w-12 text-neutral-300 dark:text-neutral-600 mx-auto mb-4" />
          <h2 className="text-lg font-semibold text-neutral-900 dark:text-white">
            {searchQuery ? 'No matching credentials' : 'No credential mappings configured'}
          </h2>
          <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
            {searchQuery
      ...
      {!isLoading && filteredMappings.length > 0 && (
        <div className="overflow-x-auto bg-white dark:bg-neutral-800 rounded-lg shadow dark:shadow-none">
                  ...
                  {isWriteUser && (
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          onClick={() => setDeleteTarget({ id: mapping.id, apiId: mapping.api_id })}
      ...
      <ConfirmDialog
        ...
        title={`Delete credential mapping for "${deleteTarget?.apiId}"?`}
        message="This action cannot be undone. The gateway will no longer inject credentials for this consumer-API pair."
        confirmLabel="Delete"
        variant="danger"
