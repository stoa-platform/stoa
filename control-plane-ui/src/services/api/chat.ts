...
export const chatClient = {
  ...
  async updateSettings(
    ...
    const { data } = await httpClient.put(path('v1', 'tenants', tenantId, 'chat', 'settings'), settings);
    return data;
  },

  async getBudgetStatus(tenantId: string): Promise<TokenBudgetStatus> {
    const { data } = await httpClient.get(path('v1', 'tenants', tenantId, 'chat', 'usage', 'budget'));
    return data;
  },

  async getUsageStats(tenantId: string, days = 30): Promise<TokenUsageStats> {
    const { data } = await httpClient.get(path('v1', 'tenants', tenantId, 'chat', 'usage', 'metering'), {
      params: { days },
    });
    return data;
  },
  ...
  async getUsageBySource(
    ...
    const { data } = await httpClient.get(path('v1', 'tenants', tenantId, 'chat', 'usage', 'tenant'), {
  ...
  async getModelDistribution(tenantId: string): Promise<ChatModelDistribution> {
    const { data } = await httpClient.get(path('v1', 'tenants', tenantId, 'chat', 'usage', 'models'));
    return data;
  },

  async createConversation(tenantId: string, title = 'New conversation'): Promise<{ id: string }> {
    const { data } = await httpClient.post(path('v1', 'tenants', tenantId, 'chat', 'conversations'), {
      title,
    });
    return data;
  },
