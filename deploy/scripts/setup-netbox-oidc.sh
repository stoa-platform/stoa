#!/usr/bin/env bash
# =============================================================================
# STOA Platform — Keycloak Netbox OIDC Client Setup
# =============================================================================
# Creates the 'stoa-netbox' client in Keycloak for Netbox SSO login.
# Pattern identical to setup-observability-oidc.sh.
#
# Usage:
#   KEYCLOAK_URL=https://auth.gostoa.dev \
#   KEYCLOAK_ADMIN_PASSWORD=xxx \
#     ./deploy/scripts/setup-netbox-oidc.sh
#
# Prerequisites:
...
#      vault kv patch stoa/vps/netbox NETBOX_OIDC_CLIENT_SECRET=<secret>
#   2. Restart Vault Agent on VPS to render new .env
#   3. Restart Netbox: docker compose down && docker compose up -d
#
...
CLIENT_ID="stoa-netbox"

NETBOX_REDIRECT_URI="https://netbox.gostoa.dev/oauth/complete/oidc/"

echo "=== STOA Netbox OIDC Client Setup ==="
...
CLIENT_PAYLOAD=$(cat <<ENDJSON
{
  "clientId": "${CLIENT_ID}",
  "name": "STOA Netbox",
  "description": "OIDC client for Netbox infrastructure cartography SSO",
  "enabled": true,
...
echo "Next steps:"
echo ""
echo "  1. Store the secret in Vault:"
echo "     vault kv patch stoa/vps/netbox NETBOX_OIDC_CLIENT_SECRET=${CLIENT_SECRET}"
echo ""
echo "  2. Restart Vault Agent on VPS to render new .env:"
echo "     ssh root@51.254.139.205 systemctl restart vault-agent"
echo ""
echo "  3. Restart Netbox on VPS:"
echo "     ssh root@51.254.139.205 'cd /opt/netbox && docker compose down && docker compose up -d'"
echo ""
echo "  4. Verify SSO: open https://netbox.gostoa.dev and check for OIDC login button"
echo ""
