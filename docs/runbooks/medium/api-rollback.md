...
curl -s https://api.dev.apim.cab-i.com/v1/deployments?api_id=<API_ID>&limit=5 | jq .
...
curl -s -H "Authorization: Bearer $AWX_TOKEN" \
  "https://awx.apim.cab-i.com/api/v2/jobs/?job_template=1&order_by=-created" | \
  jq '.results[:5] | .[] | {id, status, created}'

# 4. Check active version in Gateway
curl -s -u $GATEWAY_USER:$GATEWAY_PASSWORD \
  "https://gateway.apim.cab-i.com/rest/apigateway/apis/<API_ID>" | \
...
1. Go to https://devops.apim.cab-i.com
...
curl -X POST \
  ...
  https://api.dev.apim.cab-i.com/v1/deployments/rollback \
  -d '{
    "api_id": "<API_ID>",
...
curl -s -H "Authorization: Bearer $JWT_TOKEN" \
  https://api.dev.apim.cab-i.com/v1/deployments/<DEPLOYMENT_ID> | jq .
```
...
curl -X POST \
  ...
  -H "Content-Type: application/json" \
  https://awx.apim.cab-i.com/api/v2/job_templates/2/launch/ \
  -d '{
    "extra_vars": {
      "api_id": "<API_ID>",
      "api_name": "my-api",
...
# Follow job
curl -s -H "Authorization: Bearer $AWX_TOKEN" \
  https://awx.apim.cab-i.com/api/v2/jobs/<JOB_ID>/ | jq '{status, failed}'
```
...
curl -X PUT \
  ...
  "https://gateway.apim.cab-i.com/rest/apigateway/apis/<API_ID>/deactivate"
...
curl -X POST \
  -u $GATEWAY_USER:$GATEWAY_PASSWORD \
  -H "Content-Type: application/json" \
  "https://gateway.apim.cab-i.com/rest/apigateway/apis" \
  -d @apis/<API_NAME>/openapi.yaml

# 4. Activate restored API
curl -X PUT \
  -u $GATEWAY_USER:$GATEWAY_PASSWORD \
  "https://gateway.apim.cab-i.com/rest/apigateway/apis/<NEW_API_ID>/activate"
```
...
```bash
# Test API
curl -s https://gateway.apim.cab-i.com/gateway/<API_PATH>/health | jq .
...
# Check logs
kubectl logs -n apim deploy/apigateway --tail=50 | grep "<API_NAME>"
```
