...
- CAB-2224 Traffic Heatmap wired on branch `codex/cab-2224-traffic-heatmap` in worktree `/Users/torpedo/hlfh-repos/.worktrees/stoa-cab-2224-traffic-heatmap`. Scope UI-only: `usePrometheusRange` now preserves grouped matrix series, Live Calls adds a fixed 24h `step=3600` `sum by (http_route)` range query, top traffic routes render in `TrafficHeatmap`, and the empty state is neutral (`No route traffic in the last 24 hours.`) instead of "unavailable". Regression coverage added in `live-calls-heatmap.test.tsx`; validation passed: targeted Live Calls/Prometheus tests, full `npm run test -- --run` (2342 passed, 11 skipped), `npm run lint` (49 pre-existing warnings, 0 errors), `npm run format:check`, `npm run build` after `control-plane-ui/npm ci` + `shared/npm ci`, and browser sanity on `http://127.0.0.1:3000/observability/live-calls` redirected to `/login` with no console errors.
