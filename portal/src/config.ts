 ...
 *   1. Runtime env (container startup via runtime-config.js → window.__STOA_RUNTIME__)
 *   2. Build-time env (Vite VITE_* variables baked into the bundle)
 *   3. Hardcoded defaults
 *
 * This allows a single Docker image to be deployed to any environment
 * (local, dev, staging, prod) by setting env vars in the K8s deployment.
 *
