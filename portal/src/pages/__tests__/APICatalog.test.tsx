...
const AUDIENCE_OPTIONS: Record<PersonaRole, number> = {
  'cpi-admin': 3,
  'tenant-admin': 3,
  devops: 2,
  viewer: 1,
...
describe('APICatalog', () => {
  ...
  describe.each<PersonaRole>(['cpi-admin', 'tenant-admin', 'devops', 'viewer'])(
    ...
    (role) => {
      ...
      it('should show correct audience options count for role', () => {
        renderWithProviders(<APICatalog />);

        expect(screen.getByTestId('audience-options-count').textContent).toBe(
          String(AUDIENCE_OPTIONS[role])
        );
      });
