...
export function EnvironmentChrome({
  ...
  if (variant === 'consumer') {
    return <ConsumerChrome current={current} canonical={canonical} colors={colors} />;
  }
...
function ConsumerChrome({
  ...
  return (
    <div>
      ...
      {(isProduction || isReadOnly || canonical === 'staging') && (
        <div
          className={`flex items-center justify-center gap-2 py-1 text-xs font-medium ${colors.bg} ${colors.bgDark} ${colors.text} ${colors.textDark} ${colors.border} ${colors.borderDark} border-b`}
        >
